...
function QuestionEditor() {
    ...
    const addQuestion = () => {
        append({question: '', type: 'text', description: '', data: []});
    }
    ...
    return <div className="px-4 py-5 bg-white space-y-6 ">
        <h3 className="text-2xl font-semibold flex items-center justify-between">
            ...
            <button onClick={addQuestion} type="button" className="flex items-center text-sm py-1 px-4 rounded-sm text-white bg-gray-600 hover:bg-gray-700">
            ...
            <div key={i}>
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center">
                        <button onClick={() => handleDelete(i)} type="button" className="flex items-center text-xs py-1 px-3 rounded-sm border border-transparent text-red-500 hover:border-red-600 hover:rounded-full">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div className="my-2 grid gap-3 grid-cols-12">
                    ...
                    {shouldHaveOptions(watch(`questions.${i}.type`)) &&
                        <OptionsEditor index={i} />
                    }
                </div>
                {fields.length > i + 1 && <hr className="my-4"/>}
            </div>
        ))}
    </div>
}
